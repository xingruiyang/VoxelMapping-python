...
def main(args, map):
    train_cam = []
    test_cam = []
    total_num_imgs = len(glob(os.path.join(args.dataset, "seq-02/*.color.png")))
    ...
    for i in range(total_num_imgs):
        ...
        rgb = os.path.join(args.dataset, 'seq-02/frame-{:06d}.color.png'.format(i))
        depth = os.path.join(args.dataset, 'seq-02/frame-{:06d}.depth.png'.format(i))
        depth = cv2.imread(depth, -1)
        depth[depth > 3000] = 0
        pose = np.loadtxt(os.path.join(args.dataset, 'seq-02/frame-{:06d}.pose.txt'.format(i)))
    ...
    #     cv2.imwrite(os.path.join(args.out_path, 'seq-02/frame-{:06d}.depth.png'.format(i)), depth.astype(np.uint16))
    #     np.savetxt(os.path.join(args.out_path, 'seq-02/frame-{:06d}.pose.txt'.format(i)), pose)

    # total_num_imgs = len(glob(os.path.join(args.dataset, "seq-02/*.color.png")))
    # for i in range(total_num_imgs):
    #     if i % 10 == 0:
    #         pose = np.loadtxt(os.path.join(args.dataset, 'seq-02/frame-{:06d}.pose.txt'.format(i)))
    #         test_cam += [create_camera_actor(0.9, 0.03).transform(pose)]
    ...
    o3d.visualization.draw_geometries([mesh]+train_cam+test_cam+[points])
